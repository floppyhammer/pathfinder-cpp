...
namespace Pathfinder {
    Segment::Segment(const LineSegmentF& p_baseline, const LineSegmentF& p_ctrl)
            : baseline(p_baseline), ctrl(p_ctrl) {}

    Segment::Segment(const LineSegmentF& p_baseline, const LineSegmentF& p_ctrl,
                     SegmentKind p_kind, SegmentFlags p_flags)
    ...
    Segment Segment::to_cubic() const {
        ...
        new_segment.ctrl = LineSegmentF((baseline.from() + p1_2) / 3.0f,
                                        (p1_2 + baseline.to()) / 3.0f);
    ...
    Segment Segment::transform(Transform2 transform) const {
        return {LineSegmentF(transform * baseline.from(), transform * baseline.to()),
                LineSegmentF(transform * ctrl.from(), transform * ctrl.to()), kind, flags};
    }
    ...
    float Segment::arc_length() const {
        ...
        switch (kind) {
            ...
            case SegmentKind::Cubic: {
                ...
                auto cont_net =
                        (cubic.baseline.from() - cubic.ctrl.from()).length() + cubic.ctrl.vector().length() +
